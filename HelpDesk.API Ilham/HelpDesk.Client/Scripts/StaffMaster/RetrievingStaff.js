...
function LoadRoleCombo() {
    $.ajax({
        ...
        success: function (result) {
            var role = $('#Roles');
            role.empty();
            role.append($('<option/>').val('0').text('Select Role'));
...
function Save() {
    ...
    Staff.Dob = $('#Dob').val();
    Staff.Pob = $('#Pob').val();
    Staff.Gender = $('#Gender').val();
    Staff.Religions = $('#Religions').val();
    Staff.Villages = $('#Villages').val();
    Staff.Address = $('#Address').val();
    Staff.Phone = $('#Phone').val();
    Staff.Email = $('#Email').val();
    Staff.Roles = $('#Roles').val();
    $.ajax({
        url: 'http://localhost:11609/api/Staffs/',
        type: 'POST',
        dataType: 'json',
...
function Edit() {
    ...
    Staff.Roles = $('#Roles').val();
    $.ajax({
        url: 'http://localhost:11609/api/Staffs/' + Staff.Id,
        type: 'PUT',
        data: Staff,
        dataType: 'json',
        success: function (response) {
...
function GetById(Id) {
    debugger;
    $.ajax({
        url: 'http://localhost:11609/api/Staffs/Get/' + Id,
        type: 'GET',
        dataType: 'json',
        success: function (result) {
            console.log(result);
...
function Delete(Id) {
    swal({
        ...
        $.ajax({
            url: "http://localhost:11609/api/Staffs/" + Id,
            type: "DELETE",
            success: function (response) {
                swal({
                    title: "Deleted!",
...
function ClearScreen() {
    $('#myModal').on('hidden.bs.modal', function () {
        ...
        $('#Phone').val('');
        $('#Email').val('');
        $('#Roles').val('0'); // agar pada saat create ke 2 supplier combo box tidak kosong
        $('#Username').val('');
        $('#Password').val('');
        $('#Update').hide();
        $('#Save').show();
...
function ValidationSave() {
    ...
    if ($('#Villages').val() == "Select Village" || ($('#Village').val() == "0")) {
        isAllValid = false; //kalau textbox nama kosong maka
    ...
    if ($('#Roles').val() == "Choose Role" || ($('#Roles').val() == "0")) {
        isAllValid = false; //kalau textbox nama kosong maka
